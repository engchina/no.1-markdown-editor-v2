...
test('wysiwyg editor routes horizontal rules through the shared thematic-break detector', async () => {
  ...
  assert.match(source, /'\.cm-wysiwyg-hr': \{[\s\S]*?display: 'block'[\s\S]*?width: '100%'[\s\S]*?boxSizing: 'border-box'[\s\S]*?padding: `0 \$\{PROSE_BLOCK_INSET\}`[\s\S]*?pointerEvents: 'none'/u)
  assert.match(source, /'\.cm-wysiwyg-hr__rule': \{[\s\S]*?display: 'block'[\s\S]*?borderTop: '1px solid var\(--md-hr-rule-color, var\(--border\)\)'[\s\S]*?margin: '0'/u)
