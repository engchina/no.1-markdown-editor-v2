...
const SAMPLE_RELEASE: AvailableRelease = {
  ...
  hasUpdate: true,
  releaseUrl: 'https://github.com/engchina/no.1-markdown-editor/releases/tag/v0.12.0',
  downloadUrl: 'https://github.com/engchina/no.1-markdown-editor/releases/download/v0.12.0/app.msi',
  assetName: 'app.msi',
...
test('manual checks ignore skippedVersion while automatic checks honor it', () => {
  assert.equal(shouldShowUpdateDialog('auto', '0.12.0', '0.12.0'), false)
  assert.equal(shouldShowUpdateDialog('manual', '0.12.0', '0.12.0'), true)
  assert.equal(shouldShowUpdateDialog('auto', '0.12.0', '0.11.0'), true)
})
...
test('skipVersion stores the skipped release and closes the update dialog', () => {
  ...
  assert.equal(useUpdateStore.getState().availableRelease?.latestVersion, '0.12.0')

  useUpdateStore.getState().skipVersion('0.12.0')
  assert.equal(useUpdateStore.getState().skippedVersion, '0.12.0')
