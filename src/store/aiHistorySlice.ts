...
type AISessionHistoryPatch = Partial<
  Pick<
    AIDocumentSessionHistoryEntry,
    | 'status'
    | 'resultPreview'
...
type AIHistorySessionStartArgs = {
  tabId: string
  tabPath: string | null
  documentName: string
...
export interface AIHistorySliceDataState {
...
export interface AIHistorySliceState extends AIHistorySliceDataState {
  ...
  startSessionHistory: (args: AIHistorySessionStartArgs) => { entryId: string; threadId: string }
  updateSessionHistory: (
    tabId: string,
    tabPath: string | null,
    entryId: string,
    patch: AISessionHistoryPatch
  ) => void
...
export type AIPersistedAIHistoryState = AIHistorySliceDataState

export function createInitialAIHistoryState(): AIHistorySliceDataState {
  return {
...
function mergeSessionHistoryByDocument(
  ...
  return normalizeSessionHistoryByDocument(
    {
      ...current,
...
function normalizeAIHistoryDataState(
  value: Partial<AIHistorySliceDataState> | undefined,
  preset = sanitizeHistoryRetentionPreset(value?.historyRetentionPreset)
): AIPersistedAIHistoryState {
  const sessionHistoryByDocument = normalizeSessionHistoryByDocument(value?.sessionHistoryByDocument, preset)
  const historyCollections = normalizeHistoryCollections(value?.historyCollections, sessionHistoryByDocument)
  const historySavedViews = normalizeHistorySavedViews(value?.historySavedViews, historyCollections)

  return {
    historyRetentionPreset: preset,
    threadIdsByDocument: filterThreadIdsByDocument(value?.threadIdsByDocument ?? {}, sessionHistoryByDocument),
    sessionHistoryByDocument,
    historyCollections,
    historySavedViews,
    historyProviderRerankAudit: normalizeHistoryProviderRerankAudit(value?.historyProviderRerankAudit),
  }
}
...
function sanitizeHistoryArchive(value: unknown): AIHistoryArchive | null {
  ...
  const normalizedState = normalizeAIHistoryDataState(candidate, historyRetentionPreset)
...
export function partializeAIHistoryState<T extends AIHistorySliceDataState>(
  state: T
): AIPersistedAIHistoryState {
  return normalizeAIHistoryDataState(state, state.historyRetentionPreset)
}

export function mergePersistedAIHistoryState<T extends AIHistorySliceState>(
  persisted: Partial<AIHistorySliceDataState> | undefined,
  current: T
): T {
  return {
    ...current,
    ...normalizeAIHistoryDataState(persisted),
  } as T
}

export function createAIHistorySlice<T extends AIHistorySliceState>(
  set: SliceSet<T>,
  get: SliceGet<T>
): AIHistorySliceState {
  return {
    ...createInitialAIHistoryState(),
      ...
      set((state) => {
        ...
        const remappedCollections = state.historyCollections.map((collection) => ({
          ...
          entryRefs: collection.entryRefs.map((ref) =>
            ref.documentKey === previousKey ? { ...ref, documentKey: nextKey } : ref
          ),
        }))
        const { historyCollections, historySavedViews } = synchronizeHistoryCollectionsAndViews(
          remappedCollections,
    ...
    remapHistoryForPathChange: (oldPath, newPath) => {
      set((state) => {
        const oldPrefix = `path:${oldPath.replace(/\\/g, '/')}`
        const newPrefix = `path:${newPath.replace(/\\/g, '/')}`

        const remappedHistoryEntries = Object.entries(state.sessionHistoryByDocument).map(([documentKey, entries]) => {
          const nextDocumentKey = remapPathPrefix(documentKey, oldPrefix, newPrefix) ?? documentKey
        ...
        const remappedCollections = state.historyCollections.map((collection) => ({
          ...
          entryRefs: collection.entryRefs.map((ref) => ({
            ...ref,
            documentKey: remapPathPrefix(ref.documentKey, oldPrefix, newPrefix) ?? ref.documentKey,
          })),
        }))
        const { historyCollections, historySavedViews } = synchronizeHistoryCollectionsAndViews(
          remappedCollections,
    ...
    removeHistoryByPathPrefix: (pathPrefix) => {
      ...
      set((state) => {
        const nextSessionHistory = Object.fromEntries(
          Object.entries(state.sessionHistoryByDocument).filter(
            ([documentKey]) => !pathMatchesPrefix(documentKey, normalizedPrefix)
          )
        )
        const { historyCollections, historySavedViews } = synchronizeHistoryCollectionsAndViews(
          state.historyCollections,
      ...
      set((state) => {
        ...
        const nextSavedViews = normalizeHistorySavedViews(
          state.historySavedViews.map((view) =>
            view.collectionId === collectionId ? { ...view, collectionId: null, updatedAt: Date.now() } : view
          ),
          nextCollections
        )
    ...
    exportHistoryArchive: () => {
      ...
      return {
        version: 1,
        exportedAt: Date.now(),
        ...partializeAIHistoryState(state),
      }
    },
